"""Sanitize the Droguier massive metadata CSV without mutating the source file."""
...
RAW_PATH = Path("data/working/droguier/massive_metadata_droguier.csv")
OUTPUT_DIR = Path("data/output/droguier")
SANITIZED_PATH = OUTPUT_DIR / "massive_metadata_droguier_sanitized.csv"
IMPORT_PATH = OUTPUT_DIR / "massive_metadata_droguier_injection_import.csv"
ISSUES_PATH = OUTPUT_DIR / "massive_metadata_droguier_sanitization_issues.tsv"
SUMMARY_PATH = OUTPUT_DIR / "massive_metadata_droguier_sanitization_summary.json"
RAW_SCHEMA_PATH = OUTPUT_DIR / "massive_metadata_droguier_raw_frictionless_schema.json"
RAW_REPORT_PATH = OUTPUT_DIR / "massive_metadata_droguier_raw_frictionless_report.json"
SANITIZED_SCHEMA_PATH = OUTPUT_DIR / "massive_metadata_droguier_sanitized_frictionless_schema.json"
SANITIZED_REPORT_PATH = OUTPUT_DIR / "massive_metadata_droguier_sanitized_frictionless_report.json"
...
INTEGER_COLUMNS = {"id", "ncbi_taxonomy_id"}
SAMPLE_TYPES = {
    "sample": "sample",
    "QC": "qc",
    "blk": "blank",
...
def _augment_row(row: dict[str, str], *, row_number: int, issues: list[Issue]) -> None:
    ...
    row["file_type"] = SAMPLE_TYPES.get(row.get("sample_type", ""), row.get("sample_type", ""))
    ...
    row["normalized_date"], row["date_parse_status"] = _normalize_datetime(row.get("raw_date", ""))
...
def _normalize_datetime(raw: str) -> tuple[str, str]:
    value = raw.strip()
    if not value:
        return "", "missing"
    if value.endswith("Z"):
...
def _write_summary(
    ...
    summary: dict[str, Any] = {
        ...
        "raw_sample_type_counts": dict(Counter(row.get("sample_type", "") for row in raw_rows)),
